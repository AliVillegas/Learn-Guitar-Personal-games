...
function animateElement(element: HTMLElement | null, delay: number) {
  if (!element) return
  setTimeout(() => {
...
function handleIntersection(entry: IntersectionObserverEntry, delay: number) {
...
function useCardAnimation(delay: number) {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          handleIntersection(entry, delay)
          if (entry.isIntersecting) {
...
function LessonCard({
  ...
  const cardRef = useCardAnimation(delay)
  ...
  return (
    <section className="relative w-full mt-20 mb-12 overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4 py-16 md:py-20">
        <div className="bg-gradient-to-br from-primary/10 via-indigo-50/50 to-purple-50/50 rounded-2xl p-8 md:p-12 border-2 border-primary/20 shadow-large">
          <CTAContent />
        </div>
      </div>
    </section>
