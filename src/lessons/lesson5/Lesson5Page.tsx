...
import { useLesson5Handlers } from './useLesson5Handlers'
import { LessonRenderer, ResultRenderer } from '../common/LessonRenderer'
import { Lesson5Config } from './Lesson5Config'
...
function useConfigSync(game: ReturnType<typeof useLesson5Store>) {
  useEffect(() => {
    const syncConfig = () => {
...
function renderCompletePhase(
  score: ReturnType<typeof useLesson5Store>['score'],
...
function renderPlayingPhase(
  ...
  handlers: ReturnType<typeof useLesson5Handlers>,
...
function createLesson5Handlers(
  game: ReturnType<typeof useLesson5Store>,
  handlers: ReturnType<typeof useLesson5Handlers>
) {
...
function renderLesson5PlayingPhase(
  game: ReturnType<typeof useLesson5Store>,
  handlers: ReturnType<typeof useLesson5Handlers>,
  handleRegenerate: () => void,
...
function renderLesson5Phase(
  ...
  return renderLesson5PlayingPhase(game, handlers, handleRegenerate, handleGoToHomeWithNavigate)
}

export function Lesson5Page() {
  const navigate = useNavigate()
  const game = useLesson5Store()
  const handlers = useLesson5Handlers()

  useConfigSync(game)
  usePreloadAudioOnGameStart(game.phase)

  const { handlePlayAgain, handleGoToHome, handleRegenerate } = createLesson5Handlers(
    game,
    handlers
  )
